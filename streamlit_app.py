...
# 환경설정 & 유틸
# -----------------------------
# 로컬 개발 환경에서는 .env 사용 가능, 배포(Cloud)에서는 st.secrets 사용
try:
    load_dotenv()
except Exception:
    pass
...
def validate_env() -> bool:
    if not YOUTUBE_API_KEY:
        st.error("YOUTUBE_API_KEY가 설정되지 않았습니다. Streamlit secrets 또는 .env를 설정하세요.")
        with st.expander("secrets.toml 설정 예시"):
            st.code("""
[general]
# 일반 키-값이 아닌, 최상위에 바로 정의해도 됩니다. Streamlit Cloud에서는 UI에서 입력하세요.
...
MAX_RESULTS = 30
""", language="toml")
        with st.expander(".env 설정 예시 (로컬 개발)"):
            st.code("""
YOUTUBE_API_KEY=YOUR_API_KEY
REGION_CODE=KR
MAX_RESULTS=30
